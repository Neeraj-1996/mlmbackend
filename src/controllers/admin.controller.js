...
const updateProduct = asyncHandler(async (req, res) => {
	const { id, product_name: productName, level, ratio_between: ratioBetween, price } = req.body;
	...
	let productImgObj;
	if (productImgPath) {
		productImgObj = await uploadOnCloudinary(productImgPath);
		if (!productImgObj) {
			throw new ApiError(400, "Product Image upload failed");
		}
		product.productImg = productImgObj.url; // Update image URL
	}
